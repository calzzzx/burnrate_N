...
// On native macOS the panel resizes the OS window to fit its content. On the web
// there's no window to resize, so the panel is a fixed-size element and the list
// scrolls internally — PANEL_MAX_HEIGHT below caps it.
...
export default function Panel() {
  ...
  // Size the scrollable list to fill whatever space is left under the static
  // header/overview area, capped at PANEL_MAX_HEIGHT (no OS window to resize).
  useEffect(() => {
  ...
  }, [isLoading, view, subscriptions.length, settings.sort_by, ratesLoading, overviewMode])
  ...
  return (
    <div
      ref={panelRef}
      className={`relative w-[288px] max-h-[516px] ${!isLoading && view === 'list' ? 'h-auto' : 'h-[516px]'} bg-bg-primary rounded-[var(--radius-panel)] border border-white/[0.10] shadow-[0_12px_32px_rgba(0,0,0,0.16),inset_0_1px_0_rgba(255,255,255,0.05)] flex flex-col overflow-hidden animate-panel-in origin-top`}
    >
      {isLoading ? (
